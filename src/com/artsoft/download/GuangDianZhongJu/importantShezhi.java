...
//		mainUrl = "http://dsj.sarft.gov.cn/tims/site/views/applications/importantShezhi/view.shanty?appName=importantShezhi&id=014ee3a2d1f974484028819a4d2365a7";
//		mainurl(mainUrl);
	...
	private static void mainurl(String mainUrl) {
		...
		for (Element element : links) {
			...
			if ("margin:10px 0px 40px 0px;".equals(element.attr("style"))) {
				...
				// "内容提要",
				// false, 0);
				String strlist1 = HtmlAnalyze.getTagText(element.toString(), "制作周期", "内容提要", true, 0);
				...
				reportcompany = HtmlAnalyze.getTagText(element.toString(), "报审机构：", "</td>");
			...
			if ("border-collapse: collapse; border: medium none".equals(element.attr("style"))) {
				...
				// String strlist2 = HtmlAnalyze.getTagText(element.toString(),
				// "序号", "内容提要", true, 0);
				// // System.out.println(strlist);
