...
public class DownSohuDetalis {
	...
	public static void sohuBranch(String urlBranch) {
		...
		String name = HtmlAnalyze.getTagText(strHtml, "class=\"video_name\" title=\"", "\"");
...
//		System.out.println(times=times.replaceAll("-", ""));
		...
		String classstr = ""; // 类型:
		classstr = HtmlAnalyze.getTagText(strHtml, "类型：", "</span>");
...
//		}
	...
	public static void sohuDetailedfirst(String urlerer, String name) {
		...
		try {
			OracleOpreater.intoReputation(name, "4", Amount, "0", "", urlerer, "3", "0");
...
//					tvType, realUrl);
	...
	//免费
	private static void sohuMainMianfei(String sohuMainUrl) {
		// TODO Auto-generated method stub
		CommonUtil.setLog(TimeTest.getNowTime("yyyy-MM-dd HH:mm:ss") + ":" + sohuMainUrl);
		String strHtml = DownloadUtil.getHtmlText(sohuMainUrl, 1000 * 30, "UTF-8", null, null);
		if (strHtml == null || strHtml.equals("")) {
		...
		try {
			...
			Document doc = Jsoup.parse(strHtml);
			Elements links = doc.select("ul.st-list li div.list-hover");
			for (Element link : links) {
				TvPlay playtv=new TvPlay();
				...
				String texturl = link.select("a").attr("href");
				System.out.println(texturl);
				playtv.setTvplay_url(texturl);
				System.out.println(name);
				playtv.setTvplay_name(name);
				
				String niandai=""; //年代
				niandai=link.select("p.col3a a").attr("title");
				playtv.setShoot_time(niandai);
				
				String diqu=""; //地区
				diqu = link.select("p.col3a").text();
				String[] classlist=diqu.split("/");
				if (classlist.length==2) {
					System.out.println(diqu=classlist[1]);
					playtv.setProduction_area(diqu);
				}
				
				String classstr = ""; // 类型:
				classstr = HtmlAnalyze.getTagText(link.toString(), "class=\"lh-type\">", "</p>");
				System.out.println(classstr=classstr.replaceAll("\r\n\t", "").replaceAll("\t", ""));
				playtv.setSubject(classstr);
				
				String yanyuan="";// 演员
				String yanyuanAll= HtmlAnalyze.getTagText(link.toString(), "主演：", "</p>",true,0);
				...
				for (String stringtext : yanyuanlist) {
					...
					if (!(urlss.equals("")&&textss.equals(""))) {
						if (!urlss.contains("http://so.tv.sohu.com/")) {
							
							urlss="http://so.tv.sohu.com"+urlss;
						}
				...
				String detail="";
				detail=HtmlAnalyze.getTagText(link.toString(), "<p class=\"lh-info\">", "</p>");
				if (detail==null||detail.equals("")||detail.equals("null")) {
					detail=HtmlAnalyze.getTagText(strHtml, "<span class=\"short\" style=\"display:none;\">", "</span>");
				}
				System.out.println(detail);
				...
				// System.out.println(texthtml);
				if (name != null && !"".equals(name) && texturl != null && !"".equals(texturl)) {
//					System.out.println(texturl);
//					sohuBranch(texturl);
					playtv.setClassnum(4);
...
//		//付费
//		for (int i = 1; i <= 37; i++) {
//			sohuMain("http://so.tv.sohu.com/list_p1100_p20_p3_p40_p5_p6_p77_p80_p9_2d1_p10" + i + "_p11_p124_p13.html");
//
//		}
		//免费
		for (int i = 1; i <= 23; i++) {
			sohuMainMianfei("http://so.tv.sohu.com/list_p1100_p20_p3_p40_p5_p6_p77_p80_p9_2d1_p10" + i + "_p11_p125_p13.html");
