...
public class wangyiyule_4 {
	...
	public static void runnewMain(String urlMain) {
		...
		//
		Elements links = doc.select("div.hot-list div.item-top");
		for (Element link : links) {
			...
			String names = link.select("div.item-Text a").first().text();
			System.out.println(names);
			wechat.setNames(names);

			String DATES = link.select("h3.color-date-from").toString();// .text();
			DATES = HtmlAnalyze.getTagText(DATES, "20", "</h3>");
			DATES="20"+DATES;
...
//				if (imgurls.contains("?")) {
//					// http://cms-bucket.nosdn.127.net/0eeb412adc4a4b37bdf5911e8267bff720161201143411.jpeg
//					imgurls = HtmlAnalyze.getTagText("##" + imgurls, "##", "?", true, 0);
//				}
			...
			if (CONTENT_ALL.equals("")) {
				CONTENT_ALL = docs.select("div.post_text").toString();
			}
	...
	public static void main(String[] args) {
		// TimingTime(1, 59, 59);
		runnewMain("http://ent.163.com/star/");
