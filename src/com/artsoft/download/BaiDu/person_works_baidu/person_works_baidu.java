package com.artsoft.download.BaiDu.person_works_baidu;
...
public class person_works_baidu {
	...
	private static void huojiangjilu(String strId, String url, String strHtml, Document doc) {
		...
		String tvplay = HtmlAnalyze.getTagText(strHtml, "</span>获奖记录</h2>", "</table>", true, 0);
		// System.out.println(moves);
...
//			System.out.println(element);
			Elements mvtrtd = element.select("td li.row");
			String name = "";
			String NAME_URL = "";
			String producedTime = "";
			String Remarks = "";
			...
			for (Element element2 : mvtrtd) {
				...
				personwork.setPersonUrl(url);
				System.out.println(
						producedTime = HtmlAnalyze.getTagText(element2.toString(), "class=\"first column\">", "&nbsp;"));
				personwork.setProducedTime(producedTime);
				// }
//				if (i % 3 == 1) {
...
//				}
//				if (i % 3 == 2) {
					String listString=element2.toString();
					String[] listS=listString.split("&nbsp;&nbsp;&nbsp;");
					String remarks = "";
					if (listS.length>4) {
						remarks=listS[listS.length-3];
						System.out.println(remarks = Jsoup.parse(remarks).text());
						if (remarks.equals(personwork.getName())) {
							remarks="";
						}
						personwork.setRemarks(remarks);
						
						if (!remarks.equals("")) {
							NAME_URL =Jsoup.parse(listS[listS.length-3]).select("a").attr("href");
							System.out.println(NAME_URL);
							if (!NAME_URL.equals("")) {
								personwork.setName_URL("http://baike.baidu.com" + NAME_URL);
					...
					name = HtmlAnalyze.getTagText(element2.toString(), "<span class=\"column\">", "&nbsp;");
					if (name.equals("")) {
						name = element2.text();
//						name=name;
					}
					if (name.replace("&nbsp;", "").replace(" ", "").length()<7) {
						name=HtmlAnalyze.getTagText(element2.text(), producedTime, remarks);
					}
					personwork.setName(name);
//				}
//				i += 1;
				personwork.setDataType(8);

				personwork.setTextValue(element2.text());
				 if (!name.equals("")) {
				 OracleHaoSou.intotem_person_works(personwork);
				 }
			}
	...
	public static void mainmore(String strId, String url) {
		...
		Document doc = Jsoup.parse(strHtml);
		// 获奖记录
		huojiangjilu(strId, url, strHtml, doc);
	...
	public static void runnewMain() {
		...
		for (Object Objstring : listArray) {
			...
			System.out.println(url=listTemp.get(1));
			try {
